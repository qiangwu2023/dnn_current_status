...
def generate_case_I(n, corr, Beta):
    """
        ...
        Case 3 (Deep 1 Model): 
        g(X)=\sqrt(X1X2)/5+X3^2X4/4+\log(X4+1)/3+\exp(X5)/2-C
    ...
    # Constrain X to [0,2]
    X = np.clip(X, 0, 2)
    # ====The intercept term is approximately equal to 1.12 (by running choose_intercept.py)=====
    g_X = np.sqrt(X[:,0]*X[:,1])/5 + X[:,2]**2*X[:,3]/4 + np.log(X[:,3]+1)/3 + np.exp(X[:,4])/2 - 1.12
    Y = ndm.rand(n)
