...
def Est_deep(X_test,Z_train,X_train,U_train,De_train,Beta0,n_layer,n_node,n_lr,n_epoch,nodevec,m,c0):
    Lambda_U = I_S(m,c0,U_train,nodevec)
    C_index = 0
    for loop in range(500):
        print('deep_iteration time=', loop)
        g_X = g_D(X_test,Z_train,X_train,De_train,Lambda_U,Beta0,n_layer,n_node,n_lr,n_epoch)
        g_train = g_X['g_train']
