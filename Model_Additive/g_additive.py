...
def g_A(train_data,X_test,Lambda_U,Beta0,m0,nodevec0):
    ...
    def GA(*args):
        b = args[0]
        Lam1 = Lambda_U * np.exp( Z_train*Beta0 + np.dot(B_0, b[0:(m0+4)]) + np.dot(B_1, b[(m0+4):(2*(m0+4))]) + np.dot(B_2, b[(2*(m0+4)):(3*(m0+4))]) + np.dot(B_3, b[(3*(m0+4)):(4*(m0+4))]) + np.dot(B_4, b[(4*(m0+4)):(5*(m0+4))]) + b[5*(m0+4)]*np.ones(X_train.shape[0]))
