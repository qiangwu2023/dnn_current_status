...
def g_D(train_data,X_test,Lambda_U,Beta,Beta0,n_layer,n_node,n_lr,n_epoch):
    ...
    #%% ----------- complete training ------------
    # the best loss
    # Calculate the optimal loss with the true value of Beta
    # Lambda_U=sqrt(U)/5
    true_Lambda_U = torch.sqrt(U_train)/5
    # true_Lambda_U = U_train/5
    best_loss = my_loss(De_train, Z_train, Beta, true_Lambda_U, g_train_true)
    # print('best_loss=', best_loss.detach().numpy())
    # forecast
